...
def modelling(clean_data):
    ...
    EVC = nx.eigenvector_centrality(my_graph, max_iter=10000, weight='weight')
    result_EVC = []
    ...
    all_measure = pd.merge(df_result_BC, df_result_EVC, on='account_name', how='left')


    cols = ['BC', 'EVC']
    scaller = preprocessing.MinMaxScaler()
    all_measure[['BC_Norm', 'EVC_Norm',]] = scaller.fit_transform(all_measure[cols])
    all_measure['final_measure'] = (all_measure.BC_Norm + all_measure.EVC_Norm)/2
    all_measure = all_measure.sort_values(by='final_measure', ascending=False)

    final_result = []
    for account_name, bc, evc in zip(all_measure['account_name'], all_measure.BC, all_measure.EVC):
        buzzer = {
